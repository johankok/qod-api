...
app.get("/daily", function (req, res) {
  ...
  getConnection(res, function (connection) {
    ...
    var sql =
      "SELECT quotes.quote_id, quotes.quote, authors.author, genres.genre FROM quotes, authors, genres WHERE quote_id=? and quotes.author_id=authors.author_id and quotes.genre_id=genres.genre_id ;";
    connection.query(sql, [quote_id], function (err, rows, fields) {
      if (err) {
        ...
        if (rows.length > 0) {
          ...
          res
            .status(500)
            .json({ error: "quote id " + quote_id + " doesn't exist." });
        }
        connection.release();
      }
    });
    ...
    connection.query(sql, function (error, results, fields) {
      if (error) {
        ...
        var sql =
          "SELECT quotes.quote_id, quotes.quote, authors.author, genres.genre FROM quotes, authors, genres WHERE quote_id=? and quotes.author_id=authors.author_id and quotes.genre_id=genres.genre_id ;";
        logMsg(
          "query sql: " + sql + ", count: " + count + " quote_id: " + quote_id,
        );
        connection.query(sql, [quote_id], function (error, rows, fields) {
          if (error) {
            logErr(error);
